...
class regUseTables:
    ...
    def __init__(self, use_obj, trade_obj, tradmar_obj, suppmar_obj, va_labour_obj, va_capital_obj, va_land_obj, prodtaxes_obj, taxes_obj):
        ...
        def calc_use_bp(i, use_obj, trade_obj, tradmar_obj, suppmar_obj):
            ...
            # Use at basic prices
            use_bp = use_dp - use_dp.div(use_dp.sum(axis=1), axis = "rows").mul(margin["Trade_margin"], axis = "rows") 
        ...
        def calc_use_bp2(i, use_obj, trade_obj, tradmar_obj, suppmar_obj):
            ...
            use_bp_i = use_dp_i - use_dp_i.div(use_dp_i.sum(axis=1), axis = "rows").mul(margin["Trade_margin"], axis = "rows").fillna(0)    
                        ...
                        useTable(use = calc_use_bp(i, use_obj, trade_obj, tradmar_obj, suppmar_obj),\
                                ...
                                va = pd.DataFrame(\
                                    {va_labour_obj["coeff_name"].strip(): va_labour_obj["array"].sum(axis = 1)[:,i],\
                                     ...
                                     taxes_obj["coeff_name"].strip(): taxes_obj["array"][:,:,0:30,i].sum(axis = (0,1))}, \
